...
namespace Common
{
	...
	template<class O, class P, class List>struct __in_rect_all__
	{
		bool operator()(O *o, P *p)
		{
			...
			if(InRect(p->l.x, p->l.y, r))
			{
				...
				p->l.x = o->currentX;
				p->l.delta = 0;
				typedef VL::Erase<List, O>::Result lst;
	...
	{
		static HWND Do(HBRUSH brush = NULL)
		{
			HWND hh = FindWindow(WindowClass<T>()(), 0);
			if(NULL != hh)
			{
			...
			else
			{
				...
				hh = WindowTemplate(new T, T::Title(), r.left, r.top, r.right
					, r.bottom, IDI_UPLOAD, brush);
...
//-----------------------------------------------------------------------------------------------
	...
	//	void operator()(O *o, P *p)
	//	{
	//		typedef VL::SelectWapper<typename O::TChart::items_list, Border>::Result lst;
	//		VL::for_each<lst, __set_color__>()(&((typename O::TChart *)o->chart)->items, p);
	//		o->owner = p;
