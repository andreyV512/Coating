...
template<class Head, class ...Tail>struct __tool_bar_width__<Vlst<Head, Tail...> >
{
	static const int value = __tool_btn_width__<Head>::value + __tool_bar_width__<Vlst<Tail...>>::value;
};
template<>struct __tool_bar_width__<Vlst<>>
{
...
template<class listX, int Height = 64, int SEPARATOR_WIDTH = 10>class InitToolbar
{
	...
	HIMAGELIST himl;
	typedef listX list;
	TBBUTTON tbb[VL::Length<list>::value];
    template<class T>struct no_IDB_SEP
	{
		static const bool value = IDB_SEP != T::IDB_;
	};
public:
	typedef typename VL::EraseAllParam<list, no_IDB_SEP>::Result only_buttons_list;	
private:
	template<int>struct Set
	{
		template<class O>void Do(TBBUTTON &tbb, Self *self)
		{
			...
			tbb.iBitmap  = VL::IndexOf<only_buttons_list, O>::value;
	...
	template<class O, class P>struct loc
	{
		void operator()(P &p)
		{
			Set<O::IDB_>().Do<O>(p.tbb[VL::IndexOf<list, O>::value], &p);
		};
	};	 
...
public:
	HWND operator()(HWND hwnd)
	{
		...
		//—в¤жите image list с ToolBar -ом
...
template<class Head, class ...Tail>struct MkToolBarLst<Vlst<Head, Tail...> >
{
	typedef Vlst<
		typename VL::_if<IDB_SEP != Head::IDB_, __button_tool_bar__<Head>, Head>::Result
