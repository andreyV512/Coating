...
namespace ZoneStatus
{
	...
	template<class T, class skip>struct ÑrossLists;
	template<class skip, class Head, class ...Tail>struct ÑrossLists<Vlst<Head, Tail...>, skip>
	{
		static const bool value = VL::InList<skip, Head>::value || ÑrossLists<Vlst<Tail...>, skip>::value;
	};
	template<class skip>struct ÑrossLists<Vlst<>, skip>
	{
	...
	template<class T, class List>struct sub_del
	{
		...
		typedef typename VL::_if<ÑrossLists<erase, skipList>::value
			, Vlst<>
			, T
		>::Result Result;
	};
